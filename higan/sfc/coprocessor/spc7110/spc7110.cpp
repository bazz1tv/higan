...
auto SPC7110::read(uint24 addr, buint8 data) -> buint8 {
...
auto SPC7110::write(uint24 addr, buint8 data) -> void {
  cpu.synchronizeCoprocessors();
  if((addr & 0xff0000) == 0x500000) addr = 0x4800;  //$50:0000-ffff == $4800
  if((addr & 0xff0000) == 0x580000) addr = 0x4808;  //$58:0000-ffff == $4808
  addr = 0x4800 | (addr & 0x3f);  //$00-3f,80-bf:4800-483f
...
auto SPC7110::mcurom_read(uint24 addr, buint8 data) -> buint8 {
...
auto SPC7110::mcurom_write(uint24 addr, buint8 data) -> void {
}
...
auto SPC7110::mcuram_read(uint24 addr, buint8) -> buint8 {
  if(r4830 & 0x80) {
...
auto SPC7110::mcuram_write(uint24 addr, buint8 data) -> void {
  if(r4830 & 0x80) {
    addr = bus.mirror(addr, ram.size());
    ram.write(addr, data);
  }
