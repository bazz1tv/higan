...
auto SA1::CPUIRAM::read(uint24 addr, buint8) -> buint8 {
  cpu.synchronizeCoprocessors();
  return sa1.iram.read(addr & 0x07ff);
}

auto SA1::CPUIRAM::write(uint24 addr, buint8 data) -> void {
  cpu.synchronizeCoprocessors();
...
auto SA1::CPUBWRAM::read(uint24 addr, buint8) -> buint8 {
  cpu.synchronizeCoprocessors();
  if(dma) return sa1.dma_cc1_read(addr);
  return sa1.bwram.read(addr);
}

auto SA1::CPUBWRAM::write(uint24 addr, buint8 data) -> void {
  cpu.synchronizeCoprocessors();
