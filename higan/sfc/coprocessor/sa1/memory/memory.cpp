auto SA1::bus_read(uint24 addr, buint8 data) -> buint8 {
...
auto SA1::bus_write(uint24 addr, buint8 data) -> void {
  if((addr & 0x40fe00) == 0x002200) {  //$00-3f,80-bf:2200-23ff
...
//these ports.
auto SA1::vbr_read(uint24 addr, buint8 data) -> buint8 {
  if((addr & 0x408000) == 0x008000) {  //$00-3f,80-bf:8000-ffff
    addr = ((addr & 0x800000) >> 2) | ((addr & 0x3f0000) >> 1) | (addr & 0x7fff);
    return mmcrom_read(addr, data);
  }
...
auto SA1::read(uint24 addr) -> buint8 {
  tick();
  if(((addr & 0x40e000) == 0x006000) || ((addr & 0xd00000) == 0x400000)) tick();
  return bus_read(addr, regs.mdr);
}

auto SA1::write(uint24 addr, buint8 data) -> void {
  tick();
  if(((addr & 0x40e000) == 0x006000) || ((addr & 0xd00000) == 0x400000)) tick();
...
auto SA1::mmcrom_read(uint24 addr, buint8) -> buint8 {
...
auto SA1::mmcrom_write(uint24 addr, buint8 data) -> void {
}

auto SA1::mmcbwram_read(uint24 addr, buint8 data) -> buint8 {
...
auto SA1::mmcbwram_write(uint24 addr, buint8 data) -> void {
  if(addr < 0x2000) {  //$00-3f,80-bf:6000-7fff
    cpu.synchronizeCoprocessors();
    addr = bus.mirror(mmio.sbm * 0x2000 + (addr & 0x1fff), cpubwram.size());
...
auto SA1::mmc_sa1_read(uint addr, buint8 data) -> buint8 {
  synchronizeCPU();
  if(mmio.sw46 == 0) {
...
auto SA1::mmc_sa1_write(uint addr, buint8 data) -> void {
  synchronizeCPU();
  if(mmio.sw46 == 0) {
    //$40-43:0000-ffff x  32 projection
    addr = bus.mirror((mmio.cbm & 0x1f) * 0x2000 + (addr & 0x1fff), bwram.size());
...
auto SA1::bitmap_read(uint addr, buint8 data) -> buint8 {
  if(mmio.bbf == 0) {
    //4bpp
    uint shift = addr & 1;
...
auto SA1::bitmap_write(uint addr, buint8 data) -> void {
  if(mmio.bbf == 0) {
    //4bpp
    uint shift = addr & 1;
    addr = (addr >> 1) & (bwram.size() - 1);
