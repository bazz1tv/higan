...
auto NECDSP::read(uint24 addr, buint8) -> buint8 {
...
auto NECDSP::write(uint24 addr, buint8 data) -> void {
  cpu.synchronizeCoprocessors();
  if(addr & 1) {
...
auto NECDSP::readRAM(uint24 addr, buint8) -> buint8 {
  cpu.synchronizeCoprocessors();
  return uPD96050::readDP(addr);
}

auto NECDSP::writeRAM(uint24 addr, buint8 data) -> void {
  cpu.synchronizeCoprocessors();
