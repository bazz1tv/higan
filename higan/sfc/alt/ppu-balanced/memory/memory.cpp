...
auto PPU::vram_mmio_read(buint16 addr) -> buint8 {
  buint8 data;

  if(regs.display_disabled == true) {
    data = vram[addr];
  } else {
    buint16 v = cpu.vcounter();
    buint16 h = cpu.hcounter();
    buint16 ls = ((system.region() == System::Region::NTSC ? 525 : 625) >> 1) - 1;
...
auto PPU::vram_mmio_write(buint16 addr, buint8 data) -> void {
  if(regs.display_disabled == true) {
    vram[addr] = data;
  } else {
    buint16 v = cpu.vcounter();
...
auto PPU::oam_mmio_read(buint16 addr) -> buint8 {
  addr &= 0x03ff;
...
auto PPU::oam_mmio_write(buint16 addr, buint8 data) -> void {
  addr &= 0x03ff;
  if(addr & 0x0200) addr &= 0x021f;
...
auto PPU::cgram_mmio_read(buint16 addr) -> buint8 {
  addr &= 0x01ff;
  buint8 data;
...
auto PPU::cgram_mmio_write(buint16 addr, buint8 data) -> void {
  addr &= 0x01ff;
  if(addr & 1) data &= 0x7f;
