...
auto PPU::render_bg_tile(buint16 tile_num) -> void {
  buint8 col, d0, d1, d2, d3, d4, d5, d6, d7;

  if(color_depth == COLORDEPTH_4) {
    buint8* dest = (buint8*)bg_tiledata[TILE_2BIT] + tile_num * 64;
  ...
  if(color_depth == COLORDEPTH_16) {
    buint8* dest = (buint8*)bg_tiledata[TILE_4BIT] + tile_num * 64;
  ...
  if(color_depth == COLORDEPTH_256) {
    buint8* dest = (buint8*)bg_tiledata[TILE_8BIT] + tile_num * 64;
...
auto PPU::flush_pixel_cache() -> void {
  buint16 main = get_palette(0);
  buint16 sub  = (regs.pseudo_hires || regs.bg_mode == 5 || regs.bg_mode == 6)
              ? main
              : regs.color_rgb;
...
auto PPU::alloc_tiledata_cache() -> void {
  bg_tiledata[TILE_2BIT]       = new buint8[262144]();
  bg_tiledata[TILE_4BIT]       = new buint8[131072]();
  bg_tiledata[TILE_8BIT]       = new buint8[ 65536]();
  bg_tiledata_state[TILE_2BIT] = new buint8[  4096]();
  bg_tiledata_state[TILE_4BIT] = new buint8[  2048]();
  bg_tiledata_state[TILE_8BIT] = new buint8[  1024]();
}
