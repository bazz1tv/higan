//screen: 0 = main, 1 = sub
auto PPU::build_window_table(buint8 bg, bool screen) -> void {
  bool set = 1, clr = 0;
  buint8* table = (screen == 0 ? window[bg].main : window[bg].sub);
  ...
  const buint16 window1_left  = regs.window1_left;
  const buint16 window1_right = regs.window1_right;
  const buint16 window2_left  = regs.window2_left;
  const buint16 window2_right = regs.window2_right;
...
auto PPU::build_window_tables(buint8 bg) -> void {
