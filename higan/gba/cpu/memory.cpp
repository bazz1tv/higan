auto CPU::iwram_read(uint mode, buint32 addr) -> buint32 {
...
auto CPU::iwram_write(uint mode, buint32 addr, buint32 word) -> void {
...
auto CPU::ewram_read(uint mode, buint32 addr) -> buint32 {
  if(regs.memory.control.disable) return cpu.pipeline.fetch.instruction;
...
auto CPU::ewram_write(uint mode, buint32 addr, buint32 word) -> void {
  if(regs.memory.control.disable) return;
